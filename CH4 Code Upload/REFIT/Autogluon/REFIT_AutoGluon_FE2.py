...
# --- Feature engineering with normalized window and raw features ---
...
print("🔧 Generating features (engineered + raw normalized)...")
...
predictor.fit(train_data=df_train, tuning_data=df_val, use_bag_holdout=True, time_limit=60*6, presets='best_quality')
...
results_df.to_csv(f"{target_appliance}_predictions_1hr_1mil_Combined_AG.csv", index=False)
print("csv saved!")
...
plt.plot(time_test_full, y_true.values, label="Actual", alpha=0.6)
plt.plot(time_test_full, y_pred.values, label="Prediction (Raw)", linestyle='--', alpha=0.7)
plt.title(f"Prediction vs Actual ({target_appliance}) - Full Test Timeline (Raw Only)")
plt.xlabel("Time")
plt.ylabel("Power (W)")
plt.legend()
plt.grid(True)
