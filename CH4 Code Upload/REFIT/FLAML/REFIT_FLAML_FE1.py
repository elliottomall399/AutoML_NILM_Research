...
#df_clean = df_clean.copy() 6 MIL OPTION
...
# --- Train FLAML ---
print("\U0001f680 Training FLAML...")
automl = AutoML()
...
# General details
print("Best estimator selected:", automl.best_estimator)
...
results_df = pd.DataFrame({
    "Time": time_test_full,
    "GroundTruth": y_true.values if isinstance(y_true, pd.Series) else y_true,
    "Prediction_Raw": y_pred,
    "Prediction_PostProcessed": y_pred_filtered,
})

results_df.to_csv(f"{target_appliance}_predictions_1hr_1mil_NoSample_Feat1_FLAML.csv", index=False)
print("csv saved!")

import matplotlib.pyplot as plt

# --- Line Plot: Full Timeline (Raw + Post-Processed) ---
...
# --- Line Plot: Full Timeline (Raw Only) ---
plt.figure(figsize=(14, 5))
plt.plot(time_test_full, y_true, label="Actual", alpha=0.6)
plt.plot(time_test_full, y_pred, label="Prediction (Raw)", linestyle='--', alpha=0.7)
plt.title(f"Prediction vs Actual ({target_appliance}) - Full Test Timeline")
...
# --- Scatter Plot: Raw Prediction vs Actual ---
...
# --- Scatter Plot: Post-Processed Prediction vs Actual ---
plt.figure(figsize=(6, 6))
