...
# Resample to continuous 1-minute intervals (native rate)
df = df.reindex(pd.date_range(start=df.index.min(), end=df.index.max(), freq="1min"))

# --- Plot Grid and Solar ---
plt.figure(figsize=(14, 5))
plt.plot(df.index[:1000], df["grid"].iloc[:1000], label="Grid", alpha=0.7)
plt.plot(df.index[:1000], df["car1"].iloc[:1000], label="car1", alpha=0.7)
plt.title("Grid vs car1 (First 1000 Timesteps)")
plt.xlabel("Time")
plt.ylabel("Power (kW)")
plt.legend()
plt.grid(True)
...
plt.figure(figsize=(14, 5))
plt.plot(df.index, df["grid"], label="Grid", alpha=0.7)
plt.plot(df.index, df["car1"], label="car1", alpha=0.7)
plt.title("Grid vs car1 (Full Timeline)")
plt.xlabel("Time")
plt.ylabel("Power (kW)")
plt.legend()
plt.grid(True)
...
predictor.fit(train_data=df_train, tuning_data=df_val, use_bag_holdout=True, time_limit=600*6, presets='best_quality')
...
# --- Post-Processing: Duration Threshold ---
...
def print_metrics(y_true, y_pred, label=""):
    print(f"--- {label} ---")
    print(f"🔍 RMSE: {np.sqrt(mean_squared_error(y_true, y_pred)):.2f} kW")
    print(f"📉 MAE: {mean_absolute_error(y_true, y_pred):.2f} kW")
...
results_df.to_csv(f"{target_appliance}_pecanstreet_predictions_grid_input_1mil_1hr.csv", index=False)
print("✅ Results saved to CSV.")

# --- Plotting ---
plt.figure(figsize=(14, 5))
plt.plot(time_test_full, y_true.values, label="Actual", alpha=0.6)
plt.plot(time_test_full, y_pred.values, label="Prediction (Raw)", linestyle='--')
plt.title(f"Prediction vs Actual ({target_appliance}) - Grid Input")
plt.xlabel("Time")
plt.ylabel("Power (kW)")
plt.legend()
plt.grid(True)
