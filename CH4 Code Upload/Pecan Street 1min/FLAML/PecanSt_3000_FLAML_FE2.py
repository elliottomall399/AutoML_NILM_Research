...
def create_seq2point_dataset(df, feature_col, target_col, window_size):
    ...
    for i in range(half, len(df) - half):
        ...
        X.append(window)
        y.append(df[target_appliance].iloc[i])
...
print("🔧 Generating advanced features (FE2)...")
...
automl_settings = {
    ...
    "metric": 'mae',
    "task": 'regression',
    "log_file_name": f"flaml_{target_appliance}_FE2.log",
    "eval_method": "holdout",
    "verbose": 2,
}
automl.fit(
...
results_df.to_csv(f"{target_appliance}_FE2_FLAML_predictions_pecanstreet.csv", index=False)
