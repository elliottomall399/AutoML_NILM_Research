...
os.environ["JAVA_HOME"] = r"C:\Program Files\Java\jdk-24"  # Update this if needed
...
# --- Baseline Feature Engineering (Level 2) ---
...
df_train = pd.DataFrame([generate_baseline_features(w, t) for w, t in tqdm(zip(X_train_norm, time_raw[:val_index]))])
df_val = pd.DataFrame([generate_baseline_features(w, t) for w, t in tqdm(zip(X_val_norm, time_raw[val_index:split_index]))])
df_test = pd.DataFrame([generate_baseline_features(w, t) for w, t in tqdm(zip(X_test_norm, time_raw[split_index:]))])
...
results_df.to_csv(f"{target_appliance}_pecanstreet_H2O_baseline_predictions.csv", index=False)
...
def send_email_notification(): #Function to email on completion of script. Remove or replace with own credentials
    from_email = ""
    to_email = ""  
    subject = "Script Completed ✅"
    body = f"Your Python script for '{target_appliance}' H2O Baseline has finished running."
