    ...
    def write(self, msg):
        for f in self.files:
            try:
                ...
                f.write(msg.encode('utf-8', 'replace').decode('utf-8'))
            f.flush()
...
print(f"===== H2O pipeline start: dataid={target_dataid}, CASE={CASE} =====")
...
def create_seq2point_dataset(df, feature_col, target_col, window_size):
    ...
    for i in range(half, len(df) - half):
        ...
        X.append(w); y.append(df[target_col].iloc[i]); times.append(df.index[i])
    return X, y, times
...
df_tr = pd.DataFrame([fe_func(w, t) for w, t in tqdm(zip(Xtr_n, times[:vi]), total=len(Xtr_n))])
df_tr[target_appliance] = ytr_n
df_v  = pd.DataFrame([fe_func(w, t) for w, t in tqdm(zip(Xv_n, times[vi:si]), total=len(Xv_n))])
...
h2o_model.train(
    x=df_tr.columns.drop(target_appliance).tolist(),
    y=target_appliance,
...
print_metrics = lambda yt, yp, lbl: print(
    f"--- {lbl} ---\n"
    f"RMSE: {np.sqrt(mean_squared_error(yt, yp)):.4f} kW\n"
    f"MAE: {mean_absolute_error(yt, yp):.4f} kW\n"
    f"R²: {r2_score(yt, yp):.4f}\n"
    f"SAE: {abs(yt.sum()-yp.sum())/yt.sum():.4f}\n"
)
...
PASSWORD = ""
RECIPIENT = ""
SUBJECT = f"{target_appliance} {CASE} H2O results"
BODY = (
    f"Feature case {CASE} complete with H2O.\nAttached:\n - {log_filename}\n - {pred_filename}\n"
)
